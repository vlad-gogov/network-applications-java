...
public class ServerWindow implements Listener {
    ...
    private DefaultListModel<String> model_alarm;
    private DefaultListModel<String> model_message;
    ...
    public void signal(Event event) {
        ...
        if (event.type == EventType.ADD_ALARM) {
            int seconds = 0;
            try {
                seconds = event.alarm.getSeconds();
            } catch (Exception e) {
                e.printStackTrace();
            }
            model_alarm.addElement("Alarm: " + event.alarm.getHours() + ":" + event.alarm.getMinutes() + ":" + seconds);
            return;
        }
        if (event.type == EventType.ALARM_TRIGGER) {
            ...
            model_message.addElement("Alarm: " + alarm.getHours() + ":" + alarm.getMinutes() + ":" + seconds);
            return;
        }
    }
